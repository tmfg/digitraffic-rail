...
public class NeTExCalendarService {
        ...
        for (final Schedule schedule : schedules) {
            ...
            if (schedule.timetableType == Train.TimetableType.ADHOC) {
                ...
                if (!operatingPeriodMap.containsKey(periodId)) {
                    operatingPeriodMap.put(periodId, new NeTExOperatingPeriod(periodId, schedule.startDate, schedule.endDate));
                }
    ...
    public String createDaysOfWeekString(final Schedule schedule) {
        ...
        if (Boolean.TRUE.equals(schedule.runOnMonday)) days.add("Monday");
        if (Boolean.TRUE.equals(schedule.runOnTuesday)) days.add("Tuesday");
        if (Boolean.TRUE.equals(schedule.runOnWednesday)) days.add("Wednesday");
        if (Boolean.TRUE.equals(schedule.runOnThursday)) days.add("Thursday");
        if (Boolean.TRUE.equals(schedule.runOnFriday)) days.add("Friday");
        if (Boolean.TRUE.equals(schedule.runOnSaturday)) days.add("Saturday");
        if (Boolean.TRUE.equals(schedule.runOnSunday)) days.add("Sunday");
    ...
    public String generateDayTypeHash(final Schedule schedule) {
        ...
        if (Boolean.TRUE.equals(schedule.runOnMonday)) sb.append("Mo");
        if (Boolean.TRUE.equals(schedule.runOnTuesday)) sb.append("Tu");
        if (Boolean.TRUE.equals(schedule.runOnWednesday)) sb.append("We");
        if (Boolean.TRUE.equals(schedule.runOnThursday)) sb.append("Th");
        if (Boolean.TRUE.equals(schedule.runOnFriday)) sb.append("Fr");
        if (Boolean.TRUE.equals(schedule.runOnSaturday)) sb.append("Sa");
        if (Boolean.TRUE.equals(schedule.runOnSunday)) sb.append("Su");
