...
class GtfsControllerIntegrationTest extends MockMvcBaseTest {
    ...
    void givenDate_whenGet_thenServesThePackageGeneratedThatDay() throws Exception {
        ...
        when(generatedExportRepository.findFirstByFileNameAndCreatedGreaterThanEqualAndCreatedLessThanOrderByIdDesc(
                eq(GTFS_PASSENGER_FILENAME), any(ZonedDateTime.class), any(ZonedDateTime.class)))
        ...
        verify(generatedExportRepository).findFirstByFileNameAndCreatedGreaterThanEqualAndCreatedLessThanOrderByIdDesc(
    ...
    void givenDateWithNoGeneration_whenGet_thenNotFound() throws Exception {
        // generation failed that day, so there is no package for it — no falling back to an earlier one
        when(generatedExportRepository.findFirstByFileNameAndCreatedGreaterThanEqualAndCreatedLessThanOrderByIdDesc(
                eq(GTFS_PASSENGER_FILENAME), any(ZonedDateTime.class), any(ZonedDateTime.class)))
                .thenReturn(null);
    ...
    void givenPlainDateParam_whenGet_thenRejected() throws Exception {
        // AddableParametersFilter mirrors date into departure_date, which this endpoint does not accept
