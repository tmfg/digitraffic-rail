...
public class GeoJsonControllerAdvice implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(final Object body, final MethodParameter methodParameter, final MediaType mediaType,
        ...
        if (serverHttpRequest instanceof ServletServerHttpRequest && ((ServletServerHttpRequest) serverHttpRequest).getServletRequest().getParameter("format").equals("geojson")) {
            if (path.contains("/train-locations")) {
                return geoJsonFormatter.wrapAsGeoJson((List<TrainLocation>) body, s -> new Double[]{s.location.getX(), s.location.getY()});
            } else if (path.contains("metadata/stations")) {
                return geoJsonFormatter.wrapAsGeoJson((List<Station>) body, s -> new Double[]{s.longitude.doubleValue(), s.latitude.doubleValue()});
            }
        }
