...
public interface TrainRepository extends CustomGeneralRepository<Train, TrainId> {
    ...
    @Query("select train.id from Train train where train.id.departureDate = ?1 and (?2 = true or " + IS_NOT_DELETED + ")")
    List<TrainId> findTrainIdByDepartureDate(LocalDate date, Boolean include_deleted);
